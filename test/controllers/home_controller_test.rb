...
class HomeControllerTest < ActionDispatch::IntegrationTest
  ...
  test "lists profile names and conversation counts when logged in" do
    ...
    # Alice has two conversations (catch_up, weekly_chat); Bob has one (bob_chat).
    assert_match(/2 conversations/, response.body)
    assert_match(/1 conversation/, response.body)
